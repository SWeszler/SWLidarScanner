...
class ExportViewController: UIViewController, ARSessionDelegate {
    ...
    @IBAction func tappedExportButton(_ sender: UIButton) {
        ...
        func export(asset: MDLAsset) throws -> URL {
            ...
            let url = directory.appendingPathComponent("scan_export.obj")
        ...
        if let meshAnchors = arView.session.currentFrame?.anchors.compactMap({ $0 as? ARMeshAnchor }), let asset = convertToAsset(meshAnchors: meshAnchors) {
            do {
